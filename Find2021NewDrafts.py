...
def GetNewDrafts():
    ...
    for letter in [" "]:
       # draft=pd.read_csv(draftpath)
        #Newleagues=IDs['league_id'].map(int)
        #Newleagues=Newleagues.map(str)
        Leagues2020=mfl.get_dynasty_league_idsType1(letter,'(SF/TE',2020)
        Leagues2020=list(set(Leagues2020))
        Leagues2020=[str(league) for league in Leagues2020]
        ...
        print(Leagues2020)
        print(already)
        Newleagues=[str(league) for league in Leagues2021 if str(league) not in Leagues2020]
